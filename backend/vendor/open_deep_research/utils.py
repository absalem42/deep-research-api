...
async def tavily_search(
    ...
    # PATCH(deep-research): upstream called get_api_key_for_model() here, but that
    # function's only definition is commented out further down this file -- so this
    # line raised NameError the moment a Tavily search triggered summarisation.
    # It never fired in the reference demo because that ran with native search.
    model_api_key = configurable.user_api_key
    summarization_model = init_chat_model(
        model=configurable.summarization_model,
        max_tokens=configurable.summarization_model_max_tokens,
        api_key=model_api_key,
        base_url=configurable.model_base_url,
...
def get_tavily_api_key(config: RunnableConfig):
    """Get Tavily API key from the request config, falling back to the environment.

    PATCH(deep-research): prefer the per-request `tavily_api_key` so the service
    stays free of ambient credential state.
    """
    configurable = config.get("configurable", {}) or {}
    if key := configurable.get("tavily_api_key"):
        return key
    if api_keys := configurable.get("apiKeys", {}):
        if key := api_keys.get("TAVILY_API_KEY"):
            return key
